...
const { ParentBasedSampler, AlwaysOffSampler, AlwaysOnSampler, CompositePropagator, HttpTraceContext } = require( '@opentelemetry/core');
...
function setupTracing(serviceName){
  ...
  const provider = new NodeTracerProvider({
    ...
    propagator: new CompositePropagator([new B3Propagator({injectEncoding: B3InjectEncoding.MULTI_HEADER}), HttpTraceContext])
  });
  ...
  return { tracer: opentelemetry.trace.getTracer(serviceName), api: opentelemetry};
};
